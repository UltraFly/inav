...
TEST(Srxl2EscControlTest, RequestsTelemetryAtConfiguredFrameInterval)
{
    const srxl2EscBus_t bus = runningBus();
    srxl2EscControlScheduler_t scheduler;
    uint16_t channels[] = {1500};
    const uint16_t failsafe[] = {1000};
    uint8_t frame[SRXL2_ESC_CONTROL_FRAME_MAX_SIZE];
    srxl2EscControlSchedulerInit(&scheduler, 0, 0);
    ASSERT_TRUE(srxl2EscControlSchedulerUpdateChannels(&scheduler, channels, 1, 100, 0, 0));
    ASSERT_TRUE(srxl2EscControlSchedulerSetFailsafe(&scheduler, failsafe, 1));
    const srxl2EscControlSafety_t safety = normalSafety();
    ...
    for (unsigned frameIndex = 0;
        ...
        ASSERT_TRUE(srxl2EscControlSchedulerUpdateChannels(&scheduler, channels, 1, 100, 0, nowUs));
        ASSERT_NE((size_t)0, srxl2EscControlSchedulerBuildFrame(&scheduler, &bus,
            nowUs, &safety, NULL, 0, frame, sizeof(frame)));
...
TEST(Srxl2EscControlTest, TelemetryGrantReservesAnUninterruptedReplyWindow)
{
    const srxl2EscBus_t bus = runningBus();
    srxl2EscControlScheduler_t scheduler;
    const uint16_t channels[] = {1500};
    const uint16_t failsafe[] = {1000};
    uint8_t frame[SRXL2_ESC_CONTROL_FRAME_MAX_SIZE];
    srxl2EscControlSchedulerInit(&scheduler, 1000, 0);
    ASSERT_TRUE(srxl2EscControlSchedulerUpdateChannels(&scheduler, channels, 1, 100, 0, 1000));
    ASSERT_TRUE(srxl2EscControlSchedulerSetFailsafe(&scheduler, failsafe, 1));
    const srxl2EscControlSafety_t safety = normalSafety();
